...
public class PlayerScript : MonoBehaviour
{
	...
	private void Start()
	{
        ...
        this.zerosound.SetActive(false);
    }

	// Token: 0x060000E2 RID: 226 RVA: 0x00007B04 File Offset: 0x00005F04\
	public Vector3 position;
	public Transform player;
	public Vector3 numberBallonHoldOffset;
	public NumberBallonScript currentHeldNumberBallon;
	public Transform BalloonGoHere;
	private void Update()
	{
		//Put in Update void of PlayerScript 
		if (currentHeldNumberBallon != null)
		{
			currentHeldNumberBallon.transform.position = BalloonGoHere.transform.position;
		}
		if (Input.GetMouseButtonDown(0))
		{
			Ray ray = Camera.main.ScreenPointToRay(Input.mousePosition);
			RaycastHit raycastHit;
			if (Physics.Raycast(ray, out raycastHit) && raycastHit.collider.CompareTag("NumberBallon"))
			{
				NumberBallonScript numberBallon = raycastHit.collider.GetComponent<NumberBallonScript>();
				currentHeldNumberBallon = numberBallon;
			}
			else if (Physics.Raycast(ray, out raycastHit) && raycastHit.collider.CompareTag("MathMachine"))
			{
				MachimaticsScript mathMachine = raycastHit.collider.GetComponent<MachimaticsScript>();
				mathMachine.CheckAnswer(currentHeldNumberBallon.value);
			}
		}
		this.position = new Vector3(this.player.transform.position.x, this.player.transform.position.y, this.player.transform.position.z);
		this.MouseMove();
		this.StaminaCheck();
		this.GuiltCheck();
	...
	private void OnTriggerEnter(Collider other)
	{
		...
		if (other.transform.name == "The Test" & !this.gc.debugMode)
		{
			RenderSettings.fog = true;
			RenderSettings.fogColor = black;
			RenderSettings.fogDensity = TheTestFogDensity;
        ...
        else if (other.transform.name == "Zero Prize")
        {
            this.zerosound.SetActive(true);
        }
	}
    ...
    public GameObject zerosound;

	public float TheTestFogDensity = 0.35f;
}
